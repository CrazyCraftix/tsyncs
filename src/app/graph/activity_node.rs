...
impl ActivityNode {
    ...
    pub fn draw(&mut self, ui: &mut egui::Ui, inputs: Vec<&super::MutexNode>, outputs: Vec<&super::MutexNode>) {
        ...
        // draw arrows
        inputs.iter().for_each(|mutex| {
            ui.painter().line_segment([self.pos, mutex.pos], style.fg_stroke);
        });
        outputs.iter().for_each(|mutex| {
            ui.painter().line_segment([self.pos, mutex.pos], style.fg_stroke);
        });
